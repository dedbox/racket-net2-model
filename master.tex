...
A \emph{codec} is a composable and invertible function for structured data
serialization and other restructuring operations. A \emph{frame} is a byte
array of computable length, and \emph{framing} is the act of assembling bytes
into frames. Invertible framing functions are codecs.
...
\begin{mathpar}
  ...
  \Infer[Enc]{
    ...
    \Encode(x, f, \T) --> \Send(b, \T)
  }
\end{mathpar}

\subsection*{Primitive codecs}

\subsection*{Codec composition}

\subsection*{Composite codecs}
...
\begin{mathpar}
  ...
  \Infer[Msn]{
    ...
    \Mx |- \Messenger(f, t) --> [m |--> <f, t>]\Mx |- m
  }

  \Infer[RlsM]{
    \Mx(m) = <f, t> \and \Tx |- \Release(t) --> \Tx' |- \Void
  }{
