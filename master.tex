...
\begin{figure}
  ...
  \begin{Grammar}
    \tau
    &::=& \Listen(\tau) & bind authority \\
    & | & \Accept(\tau) & accept connection \\
    & | & \Connect(\tau) & connect to authority \\
    & | & \Release(\tau) & unbind / disconnect \\
    & | & \Send(\tau, \tau) & send bytes \\
    & | & \Receive(\tau) & receive bytes \\
  \end{Grammar}
  \hfill
  \begin{Grammar}
    v
